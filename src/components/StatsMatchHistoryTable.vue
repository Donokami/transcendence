    ...
    <table class="table w-full">
      ...
      <tbody>
        ...
        <tr v-for="match in matchHistoryData" :key="match.id" v-else>
          <td class="text-center">{{ match.playerA.username }}</td>
          <td class="text-center">{{ `${match.scoreA} - ${match.scoreB}` }}</td>
          <td class="text-center">{{ match.playerB.username }}</td>
...
const getMatchHistoryData = async (): Promise<void> => {
  const data: Match[] = await fetcher.get(`/games/matchs/${props.userId}`)
